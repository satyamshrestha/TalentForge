...
def test_create_interview_from_resume_success():
    ...
    service.interview_repository.create_interview.return_value = interview
    service.interview_repository.get_interview_by_id.return_value = interview
...
def test_retake_interview_success():
    ...
    assert result.role_target == original_interview.role_target
    assert result.status == "CREATED"

    service.interview_repository.create_interview.assert_called_once()

    service.question_repository.create_question.assert_called_once()

    created_question = (
        service.question_repository.create_question.call_args.args[1]
    )

    assert (
        created_question.question_text
        == "Explain dependency injection."
    )
